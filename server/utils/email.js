...
/**
 * 获取 email 列表
 * @param {Object} target - comment { replies: [....] }
...
exports.getEmailData = (article, discussData, userId) => {
  ...
  const HTML_CONTENT = `
    ${createDisscusInfo(discussData)}
    <div style="padding-left: 15px;">${discussData.replies.map(createDisscusInfo).join('')}</div>
  `;
  ...
  return { html, emailList: getEmailList(discussData, userId) };
};
